...
import {
  useCreateTractMapMutation,
  useCreateAssessorMapMutation,
  useCreateRunsheetMutation,
  useCreateStarterMutation,
  useCreateTitleChainDocMutation,
...
/* form key → API field name per section */
const FORM_TO_API: Record<string, Record<string, string>> = {
  "Tract Map": {
    tractNo: "tractNo",
    bookPage: "bookPage",
    recDate: "recordedDate",
...
export default function SectionUploadModal({
  ...
  const [createTractMap] = useCreateTractMapMutation();
  const [createAssessorMap] = useCreateAssessorMapMutation();
  const [createRunsheet] = useCreateRunsheetMutation();
  const [createStarter] = useCreateStarterMutation();
  const [createTitleChainDoc] = useCreateTitleChainDocMutation();
  ...
  const buildFormData = (apiFieldMap: Record<string, string>): FormData => {
    const fd = new FormData();
    for (const [formKey, apiKey] of Object.entries(apiFieldMap)) {
  ...
  const handleSave = async () => {
    ...
    setUploading(true);
    try {
      const apiMap = FORM_TO_API[section] || {};

      if (section === "Tract Map") {
        await createTractMap({ orderId, data: buildFormData(apiMap) }).unwrap();
      } else if (section === "Assessor Map") {
        await createAssessorMap({ orderId, data: buildFormData(apiMap) }).unwrap();
      } else if (section === "Runsheet") {
        await createRunsheet({ orderId, data: buildFormData(apiMap) }).unwrap();
      } else if (section === "Title Chain Review") {
        await createTitleChainDoc({ orderId, data: buildFormData(apiMap) }).unwrap();
      } else if (section === "Starters") {
        await createStarter({ orderId, data: buildFormData(apiMap) }).unwrap();
      } else {
        /* Assessor Page and Tax Cert have no section-specific upload endpoint */
        toast.error(`Upload not supported for "${section}" from this modal.`);
        setUploading(false);
        return;
      }
    ...
    >
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-5">
          ...
          <div className="mb-4.5">
            ...
            >
              ...
              {UPLOAD_CATS.filter((c) =>
                ["Tract Map", "Assessor Map", "Runsheet", "Title Chain Review", "Starters"].includes(c.label)
              ).map((c) => (
        ...
        <div className="border-t border-[#f1f5f9] px-5 py-3 flex items-center justify-between shrink-0">
          ...
          <button
            onClick={handleSave}
            disabled={!section || !orderId || uploading}
