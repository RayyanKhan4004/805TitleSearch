      ...
      <div className="flex flex-col gap-3.5">
        <div className={S.card}>
          ...
          <div className="p-4 flex flex-col gap-[7px]">
            ...
            <div className="flex items-center gap-2.25 mt-0.5"><div className="flex-1 h-1.5 rounded-full overflow-hidden" style={{ background: "var(--bg-page)" }}><div className="h-1.5 rounded-full transition-[width] duration-500" style={{ width: pct + "%", background: "var(--status-success-emerald)" }} /></div><span className="text-[11px] font-semibold text-text-secondary whitespace-nowrap">{checks.filter((c) => c.ok).length}/{checks.length} ready</span></div>
          </div>
        </div>
        <div className={S.card}>
          <div className="p-[18px] flex flex-col gap-2.25">
            ...
            <button className={`${S.white} w-full justify-center px-4 py-2.5 rounded-lg flex items-center gap-1.5`}><Icon name="save" size={12} />Save &amp; Close Order</button>
            <button className={`${S.red} w-full justify-center px-4 py-3 text-[12px] rounded-lg flex items-center gap-1.5`} style={{ background: "var(--status-success-emerald)" }}><Icon name="checkCircle" size={13} />Submit Order</button>
          </div>
        </div>
      </div>
    </div>
