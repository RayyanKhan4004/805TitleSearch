...
export default function StepReview({ onSaveClose }: StepReviewProps) {
  ...
  const toggleCheck = (i: number) => {
    setChecks((prev) => prev.map((c, j) => (j === i ? { ...c, ok: !c.ok } : c)));
  };
      ...
      <div className="flex flex-col gap-3.5">
          ...
          <div className="p-4 flex flex-col gap-[7px]">
            {checks.map((c, i) => (
              <div
                ...
                className={`flex items-center gap-2.25 px-[11px] py-2 rounded-lg border cursor-pointer transition-all duration-150 ${c.ok ? "border-status-success-border bg-status-success-bg" : "border-status-warning-border bg-status-warning-bg"}`}
              >
                <Icon name={c.ok ? "checkCircle" : "alertTri"} size={14} className={`shrink-0 ${c.ok ? "text-status-success-emerald" : "text-status-warning-text"}`} />
                <span className={`text-[11px] font-medium ${c.ok ? "text-status-success-text" : "text-status-warning-text"}`}>{c.label}</span>
              </div>
            ))}
        ...
        <Card>
          <div className="p-[18px] flex flex-col gap-2.25">
            ...
            <Button variant="secondary" className="w-full justify-center px-4 py-2.5" onClick={onSaveClose}>
