...
import {
  ...
  useCreateTaxCertMutation,
  useDeleteTaxCertMutation,
  usePatchTaxCertMutation,
...
function initChips(
  list?: { id?: number; code: string; verbiage: string }[],
  selected = false,
): Chip[] {
...
export default function TaxCertCard({
  ...
  const [chips, setChips] = useState<Chip[]>(() => initChips(initialAddedCodes, readOnly));
  ...
  const [reorderTaxCerts] = useReorderTaxCertsMutation();
  ...
  useEffect(() => {
    if (initialAddedCodes && initialAddedCodes.length > 0)
      setChips(initChips(initialAddedCodes, readOnly));
  }, [initialAddedCodes, readOnly]);
  ...
  /* ── drag reorder ── */
  const moveChip = (fromIdx: number, toIdx: number) => {
    if (fromIdx === toIdx) return;
    const source = readOnly ? chips : selectedChips;
    const next = [...source];
    ...
    if (readOnly) {
      setChips(next);
      if (orderId) {
        const items = next.filter((c) => c.apiId).map((c, i) => ({ id: c.apiId!, sortOrder: i + 1 }));
        if (items.length > 0)
          reorderTaxCerts({ orderId, items }).unwrap().catch(() => toast.error("Failed to save tax cert order"));
      }
    } else {
      // rebuild full chips array preserving non-selected positions
      const newChips = [...chips];
      const selIndices = newChips.reduce<number[]>((acc, c, i) => { if (c.selected) acc.push(i); return acc; }, []);
      next.forEach((chip, i) => { newChips[selIndices[i]] = chip; });
      setChips(newChips);
      if (orderId) {
        const items = newChips.filter((c) => c.apiId).map((c, i) => ({ id: c.apiId!, sortOrder: i + 1 }));
        if (items.length > 0)
          reorderTaxCerts({ orderId, items }).unwrap().catch(() => toast.error("Failed to save tax cert order"));
      }
    }
  ...
  const addFromSearch = async (code: string, verbiage: string) => {
    ...
    try {
      ...
      toast.error(err?.status === 409 ? `Code ${code} already exists` : "Failed to create tax cert");
    }
    setQuery("");
    setShowDropdown(false);
  };
  ...
  /* ── render list for readOnly vs full-edit ── */
  const listToRender = readOnly ? chips : selectedChips;
    ...
    <div className="rounded-xl" style={{ border: "1px solid var(--border-primary)", background: "var(--bg-card)" }}>
      ...
      >
        <div className="flex items-center gap-2.5">
          ...
          {(readOnly ? chips : selectedChips).length > 0 && (
            <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full ml-1 bg-status-success-bg text-status-success-text border border-status-success-border">
              {readOnly ? `${chips.length} ${chips.length === 1 ? "code" : "codes"}` : `${selectedChips.length} selected`}
            </span>
          )}
        </div>
              ...
              <div className="flex flex-wrap gap-1.75">
                {chips.map((chip) => (
                  ...
                  >
                    ...
                    {chip.selected && <Icon name="checkCircle" size={10} className="text-status-success-text" />}
                    <span
            ...
            <div className="flex flex-col gap-2">
              ...
              {listToRender.map((c, idx) => (
                <div
                  ...
                  onDrop={() => {
                    if (dragIdx !== null && dragIdx !== idx) moveChip(dragIdx, idx);
                    setDragIdx(null);
                    setDragOverIdx(null);
                  }}
                ...
                >
                  ...
                  <div
                    ...
                    onClick={() => !readOnly && !c.editing && toggleSelect(c.id)}
                  >
                    <div className="flex items-center gap-2">
                      <span
                        className="cursor-grab active:cursor-grabbing text-[#cbd5e1] hover:text-[#94a3b8] transition-colors flex"
                  ...
                  <div className="px-3 py-2.5">
                    {!readOnly && c.editing ? (
                      ...
                      <div style={{ fontSize: 11, color: "var(--color-text)", lineHeight: 1.6, whiteSpace: "pre-wrap", fontFamily: "'Times New Roman',serif" }}>
                        {c.verbiage ? renderVerbiage(c.verbiage) : <span style={{ color: "#94a3b8" }}>No verbiage.</span>}
                      </div>
                    )}
                  </div>
                </div>
