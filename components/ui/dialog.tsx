...
function Dialog({ open, onOpenChange, children }: DialogProps) {
  if (!open) return null;
  return (
    <div className={cn(dialogOverlay)} style={{ background: "var(--modal-overlay)" }} onClick={() => onOpenChange(false)}>
    ...
    >
      {(title || onClose) && (
        <div className="bg-header px-[22px] py-[14px] flex items-center justify-between flex-shrink-0">
          <div>
            {title && <div className="text-[14px] font-bold text-white">{title}</div>}
...
const DialogFooter = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn("border-t border-border px-[22px] py-2.75 flex justify-between items-center flex-shrink-0", className)}
      {...props}
    />
  )
);
...
const DialogTitle = React.forwardRef<HTMLParagraphElement, React.HTMLAttributes<HTMLParagraphElement>>(
  ({ className, ...props }, ref) => (
    <p
      ref={ref}
